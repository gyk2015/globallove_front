<template>
	<div class="mood">
		<div class="publish-area">
			<el-form ref="newMood" :model="newMood">
				<el-form-item label="">
				    <el-input type="textarea" v-model="newMood.content"></el-input>
				</el-form-item>
				<el-form-item>
	    ...
	    methods: {
	    	...
	    	onSubmit() {
	    		this.$http({
					...
					url: 'mood/addMood',
					data: {
						"content": this.newMood.content,
					}
				}).then((response)=>{
					if(response && response.data.status == 'SUCCESS') {
						this.$message(response.data.message);
						this.newMood.content = "";
						this.initMood(1);
					}else {
						this.$message(response.data.message);
					}
